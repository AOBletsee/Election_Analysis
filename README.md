...
The election_analysis.txt file can be found in this repository: [Election Analysis text output](Analysis/election_analysis.txt).

In the bulleted items below, we will break down the answers to each question about the results obtained, and how we were able to achieve them using Python coding. Please note that, while transcription of code to this Read.me file is merely illustrative, all of the coding in its integrity can be vizualized in the original file found in this repository: [Election Analysis code](PyPoll_Challenge.py).
